...
class Camera:
    ...
    def recarregar(self, faces_vertices):
        ...
        #pontos_tela = self.rasterizacao_poligono(faces_vertices_tela)
        return pontos_tela

    #!
    def rasterizacao_poligono(self, faces_vertices_tela):
        pontos = []
        for face in faces_vertices_tela:
            A, B, C = self._vertice_y_menor(face)
            linhaAB = self._rasterizacao_linha(A, B)
            linhaAC = self._rasterizacao_linha(A, C)
            linhaBC = self._rasterizacao_linha(B, C)
            pontos_aux = linhaAB + linhaAC + linhaBC
            for y in range(A[1], C[1]):
                pontos_horizontal = [(x1,y1) for x1,y1 in pontos_aux if y1 == y]
                pontos += pontos_horizontal
                pontos_horizontal_aux = list(pontos_horizontal)
                while len(pontos_horizontal_aux):
                    ponto = pontos_horizontal_aux.pop()
                    if len(pontos_horizontal_aux):
                        ponto_aux = pontos_horizontal_aux.pop()
                        step = 1 if ponto[0] <= ponto_aux[0] else -1
                        for x in range(ponto[0], ponto_aux[0] + step, step):
                            pontos.append((x, y))
    ...
    def rasterizacao_triangulos(self, faces_vertices_tela):
        ...
        for face in faces_vertices_tela:
            ...
            if not deltay1_0 == 0:
                ...
                stepx = 1 if deltax1_0 >= 0 else -1
                for y in range(x0_y0[1], x1_y1[1] + 1):
                    if x_min <= x_max:
                        for x in range(round(x_min), round(x_max) + stepx, stepx):
                            pontos.append((x,y))
                    else:
                        for x in range(round(x_max), round(x_min) + stepx, stepx):
                            pontos.append((x,y))
                    x_min += declive1_0
                    x_max += declive2_0

                x3_y3 = (round(x_max),x1_y1[1])
                deltax1_2 = x1_y1[0] - x2_y2[0]
                deltay1_2 = x1_y1[1] - x2_y2[1]
                deltax3_2 = x3_y3[0] - x2_y2[0]
                deltay3_2 = x3_y3[1] - x2_y2[1]

                stepx = 1 if deltax1_2 >=0 else -1
                if not deltay1_2 == 0:
                    x_min = x2_y2[0]
                    x_max = x2_y2[0]
                    ...
                    for x in range(x2_y2[0], x1_y1[0] + stepx, stepx):
                        pontos.append((x, x2_y2[1]))
            else:
                for x in range(x0_y0[0], x1_y1[0]):
    ...
    #OK!
    def _rasterizacao_linha(self, verticeU, verticeV):
        pontos_in_linha = []
        deltax = verticeV[0] - verticeU[0]
        deltay = verticeV[1] - verticeU[1]
        if not deltax == 0:
            error = 0
            declive = abs(deltay / deltax)
            y = verticeU[1]
            step = 1 if verticeU[0] < verticeV[0] else -1
            soma_y = 1 if verticeU[1] < verticeV[1] else -1
            for x in range(verticeU[0], verticeV[0], step):
                pontos_in_linha.append((x,y))
                error += declive
                if error >= 0.5:
                    y += soma_y
        else:
            x = verticeU[0]
            step = 1 if verticeU[1] < verticeV[1] else -1
            for y in range(verticeU[1], verticeV[1], step):
                pontos_in_linha.append((x, y))
        return pontos_in_linha
